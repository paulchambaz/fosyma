...
public class RandomWalkBehaviour extends TickerBehaviour {
  ...
  public void onTick() {
    ...
    if (myPosition != null) {
      ...
      try {
        System.out.println(
            "Press enter in the console to allow the agent "
      ...
      if (b) {
        ...
        System.out.println(
            this.myAgent.getLocalName()
                + " - State of the observations after trying to pick something "
                + lobs2);
      }
      ...
      int moveId =
          1
              + r.nextInt(
                  lobs.size()
                      - 1); // removing the current position from the list of target, not necessary
      // as to stay is an action but allow quicker random move
