...
/**
 * This class is used to start the platform and the agents. To launch your agents in the environment
 * you desire you will have to :
 *
 * <ul>
 *   <li>set the ConfigurationFile parameters (and maybe update the files in resources/) {@link
 *       ConfigurationFile}
...
public class Principal {
  ...
  private static HashMap<String, ContainerController> containerList =
      new HashMap<String, ContainerController>(); // container's name - container's ref
  ...
  public static void main(String[] args) {
    ...
    if (ConfigurationFile.COMPUTERisMAIN) {
      // We should create the Platform and the GateKeeper, whether the platform is distributed or
      // not

      // 1), create the platform (Main container (DF+AMS) + containers + monitoring agents : RMA and
      // SNIFFER)
      rt = emptyPlatform(containerList);

      // 2) create the gatekeeper (in charge of the environment), the agents, and add them to the
      // platform.
      ...
      // 1') If a distant platform already exist and you want to create and connect your container
      // to it
      containerList.putAll(
   ...
   * @return an Hmap associating the name of a container and its object reference. <br>
   *     note: there is a smarter way to find a container with its name, but we go fast to the goal
   *     here. Cf jade's doc.
   */
  ...
  /**
   * @param containerName
   * @param host is the IP of the host where the main-container should be listen to. A null value
   *     means use the default (i.e. localhost)
   * @param platformID is the symbolic name of the platform, if different from default. A null value
   *     means use the default (i.e. localhost)
  ...
  private static List<AgentController> createAgents(
    ...
    if (ConfigurationFile.COMPUTERisMAIN) {
      ...
      try {
        ...
        // Object[] objtab=new Object[]{null,null,ConfigurationFile.ENVIRONMENT_TYPE};// used to
        // give informations to the agent
    ...
    // 				Assert.assertNotNull("This container does not exist",c);
    //
    // 				// 2) Give the name of your agent, MUST be the same as the one given in the entities
    // file.
    // 				agentName="ImHere";
    //
    ...
    ag =
        createNewDedaleAgent(
            c,
            agentName,
            ExploreCoopAgent.class.getName(),
            entityParametersExplo1); // ExploreSoloAgent
    agentList.add(ag);
    ...
    ag =
        createNewDedaleAgent(
            ...
            agentName,
            ExploreCoopAgent.class.getName(),
            entityParametersExplo2); // ExploreSoloAgent
    agentList.add(ag);
    ...
    //
    // 		// 4) Give the class name of your agent to let the system instantiate it
    // 		ag=createNewDedaleAgent(c, agentName, ExploreCoopAgent.class.getName(),
    // entityParametersExplo4);// ExploreSoloAgent
    //
    // 		agentList.add(ag);
    ...
    // 		// 4) Give the class name of your agent to let the system instantiate it
    // 		ag=createNewDedaleAgent(c, agentName, ExploreCoopAgent.class.getName(),
    // entityParametersExplo5);// ExploreSoloAgent
    //
    // 		agentList.add(ag);
    /***********************************************************************
    ...
    //
    // 		// 4) Give the class name of your agent to let the system instantiate it
    // 		ag=createNewDedaleAgent(c, agentName, DummyCollectorAgent.class.getName(),
    // entityParametersC);
    // 		agentList.add(ag);
    //
    // 		// 1) Get the container where the agent will appear
  ...
  private static AgentController createNewDedaleAgent(
      ContainerController initialContainer,
      String agentName,
      String className,
      Object[] additionnalParameters) {
    // Object[] objtab=new Object[]{env,agentName};// used to give informations to the agent
    Object[] objtab =
        AbstractDedaleAgent.loadEntityCaracteristics(
            agentName, ConfigurationFile.INSTANCE_CONFIGURATION_ENTITIES);
