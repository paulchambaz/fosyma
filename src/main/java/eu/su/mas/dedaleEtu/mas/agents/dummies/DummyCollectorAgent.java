...
/**
 * This dummy collector moves randomly, tries all its methods at each time step, store the treasure
 * that match is treasureType in its backpack and intends to empty its backPack in the Tanker
 * agent. @see {@link RandomWalkExchangeBehaviour}
 *
 * @author hc
 */
public class DummyCollectorAgent extends AbstractDedaleAgent {
  ...
  /**
   * This behaviour is triggerable every 600ms. It tries all the API methods at each time step,
   * store the treasure that match the entity treasureType in its backpack and intends to empty its
   * backPack in the Tanker agent (if he is in reach)
   *
   * <p>Rmq : This behaviour is in the same class as the DummyCollectorAgent for clarity reasons.
   * You should prefer to save your behaviours in the behaviours package, and all the behaviours
   * referring to a given protocol in the same class
   *
   * @author hc
   */
  class RandomWalkExchangeBehaviour extends TickerBehaviour {
    ...
    public void onTick() {
      ...
      if (myPosition != null && myPosition.getLocationId() != "") {
        ...
        //				try {
        //					System.out.println("Press enter in the console to allow the agent
        // "+this.myAgent.getLocalName() +" to execute its next move");
        //					System.in.read();
        //				} catch (IOException e) {
        //					e.printStackTrace();
        //				}
            ...
            case GOLD:
              System.out.println(
                  this.myAgent.getLocalName()
                      + " - My treasure type is : "
                      + ((AbstractDedaleAgent) this.myAgent).getMyTreasureType());
              System.out.println(
                  this.myAgent.getLocalName()
                      + " - My current backpack capacity is:"
                      + ((AbstractDedaleAgent) this.myAgent).getBackPackFreeSpace());
              System.out.println(
                  this.myAgent.getLocalName()
                      + " - I try to open the safe: "
                      + ((AbstractDedaleAgent) this.myAgent).openLock(Observation.GOLD));
              System.out.println(
                  this.myAgent.getLocalName()
                      + " - Value of the treasure on the current position: "
                      + o.getLeft()
                      + ": "
                      + o.getRight());
              System.out.println(
        ...
        int moveId =
            1
                + r.nextInt(
                    lobs.size()
                        - 1); // removing the current position from the list of target to accelerate
        // the tests, but not necessary as to stay is an action
